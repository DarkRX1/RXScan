...
Target / Scope -> ScanPlan Compiler -> Task Lowering -> Reactive Scheduler <-> Speed + Budgets
                                                     <-> Backpressure / Queue
                                                     -> HostDiscovery + TcpDiscovery + ServiceProbe + WebProbe + Crawl + Baseline + ContentDiscovery + Fuzz Modules -> Typed Events / Evidence / Assets / Findings
...
`cli` captures operator intent (`--ports`/`--all-ports`/`--ping`/`--discover` preserved; `--max-hosts` bounds CIDR; `-w/--wordlist` streams managed content candidates at L4+). `config` resolves explicit global then project TOML configuration with CLI precedence. `target`, `scope`, and `plan` normalize seeds, enforce deny-by-default scope, and create immutable `ScanPlan`s. `execution` owns scheduler admission, canonical task IDs, speed, budgets, cancellation, retries, output retention, and best-effort follow-up admission. Native modules perform host discovery, TCP connect scanning, service observation, HTTP/TLS probing, bounded crawling, Phase 10 baseline characterization, Phase 11 managed content discovery, and Phase 12 contextual GET query fuzzing. `decision` is the only component that turns facts into follow-up tasks: host→port, open-port→service, service→web, confirmed/crawled endpoint→crawl/baseline, origin baseline→content, baseline signature with observed safe query parameter→fuzz. `web` remains the only HTTP/TLS transport and URL canonicalization layer; `crawl`, `baseline`, `content`, and `fuzz` reuse it. Compact scan-lifetime registries track contacted canonical GET URLs, origin missing-resource baselines, and bounded signature representatives; these are optimization/correlation state, not authorization. `content` streams a small built-in/user candidate source, normalizes candidates into the same origin, dedups canonical URLs, applies Phase 10 baseline filtering, and emits typed content evidence. `fuzz` mutates only already-observed non-sensitive GET query parameters one at a time with inert values and emits behavioral delta evidence, not vulnerabilities. Still deferred: UDP, DNS execution, generic exploit fuzzing, POST/form submission, cookie/header/path-variable fuzzing, JavaScript execution, headless browsers, vulnerability checks, exploitation, cipher enumeration, and technology fingerprinting.
