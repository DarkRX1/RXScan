...
Target / Scope -> ScanPlan Compiler -> Task Lowering -> Reactive Scheduler <-> Speed + Budgets
                                                     <-> Backpressure / Queue
                                                     -> HostDiscovery + TcpDiscovery + ServiceProbe + WebProbe + Crawl + Baseline + ContentDiscovery Modules -> Typed Events / Evidence / Assets / Findings
                                                     -> JSONL Output + human service summary -> Decision Engine (host→port, open-port→service, service→web→crawl→baseline→content)
...
`cli` captures operator intent (`--ports`/`--all-ports`/`--ping`/`--discover` preserved; `--max-hosts` bounds CIDR; `-w/--wordlist` streams managed content candidates at L4+). `config` resolves explicit global then project TOML configuration with CLI precedence. `target`, `scope`, and `plan` normalize seeds, enforce deny-by-default scope, and create immutable `ScanPlan`s. `execution` owns scheduler admission, canonical task IDs, speed, budgets, cancellation, retries, output retention, and best-effort follow-up admission. Native modules perform host discovery, TCP connect scanning, service observation, HTTP/TLS probing, bounded crawling, Phase 10 baseline characterization, and Phase 11 managed content discovery. `decision` is the only component that turns facts into follow-up tasks: host→port, open-port→service, service→web, confirmed/crawled endpoint→crawl/baseline, origin baseline→content. `web` remains the only HTTP/TLS transport and URL canonicalization layer; `crawl`, `baseline`, and `content` reuse it. Compact scan-lifetime registries track contacted canonical GET URLs, origin missing-resource baselines, and bounded signature representatives; these are optimization/correlation state, not authorization. `content` streams a small built-in/user candidate source, normalizes candidates into the same origin, dedups canonical URLs, applies Phase 10 baseline filtering, and emits typed content evidence. `model` is the interchange boundary, with typed content events and relationships (`DiscoveredByContentProbe`, `DerivedFromCandidateSource`) alongside crawler and baseline relationships. Still deferred: UDP, generic fuzzing, contextual parameter mutation, form submission, JavaScript execution, headless browsers, DNS execution, vulnerability checks, exploitation, cipher enumeration, and technology fingerprinting.
...
- Web/crawl/baseline/content: `web` (canonical URLs, HTTP/1.1 exchange, bounded redirects, response bounds, web policy), `web_probe` (`HttpProbe` scheduler module: explicit-URL and default-root paths, endpoint/certificate assets, per-URL evidence/findings), `extract` (pure bounded HTML/robots/sitemap/JS extraction), `crawl` (`Crawl` scheduler module, endpoint graph events/relationships, policy/budgets), `baseline` (`Baseline` scheduler module, response signatures, scan-lifetime origin baselines, bounded duplicate/similarity representatives, soft-404/wildcard observations, classes, parameter records, interestingness), `content` (`ContentDiscovery` scheduler module, built-in/user-file candidate streaming, canonical plus scan-lifetime contacted-URL dedup, baseline-aware classification)
