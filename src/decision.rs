//! Phase 6 Decision Engine V1: host facts → bounded port proposals.
//!
//! Boundary (modules never self-schedule):
//! `HostDiscovery result -> Decision Engine -> proposed TcpDiscovery task ->
//! Scope Guard -> Policy/Budget validation -> Scheduler admission`.
//!
//! Default policy (explicit, documented):
...
//!
//! The engine pre-validates scope; the scheduler re-validates admission,
//! budgets, and duplicates. Proposals carry the same scope/params/priority/
//! timeout/retry/module as lowering so single-host duplicates share task IDs
